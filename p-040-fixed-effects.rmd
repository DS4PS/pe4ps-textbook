...
# dat.ols <- data.frame( corn=y1, fertilizer=x1, org.id=paste0( "farm-", id ) )
# dput( dat.ols )
...
# dat.re <- data.frame( corn=y2, fertilizer=x2, org.id=paste0( "farm-", id ) )
# dput( dat.re )
...
# dat.fe <- data.frame( corn=y3, fertilizer=x3, org.id=paste0( "farm-", id ) )
# dput( dat.fe )
...
org.id <- dat.ols$org.id 

x1 <- dat.ols$fertilizer
y1 <- dat.ols$corn

x2 <- dat.re$fertilizer
y2 <- dat.re$corn

x3 <- dat.fe$fertilizer
y3 <- dat.fe$corn
...
# data = data.frame( G1, G2, G3, G4, G5, G6, G7, Y1, Y2, Y3, Y4, Y5, X, Outcome )

G = as.vector(cbind(rep(1, 5), rep(2, 5), rep(3, 5), rep(4, 5), rep(5, 5), rep(6,5), rep(7,5)))
Y  = rep(c(2010:2014), 7)

# data = data.frame( data, G, Y )

data = data.frame( G1, G2, G3, G4, G5, G6, G7, Y1, Y2, Y3, Y4, Y5, X, Outcome, G, Y )
```
...
data <- data.frame( Company, Year, RDPub, RD )

# write.csv( data, "research-and-dev-fe-example.csv", row.names=F )
```
