...
* **wooldridge** contains econometric datasets 
...
* How do you build a regression model when your outcome is binary (yes/no or 1/0)? 
* When is a linear probability model appropriate? 
* What is the difference between a linear probability model and a logit model?
* Why do we use a logit model?
* How do we interpret results from a logit model?
...
z = 1 + 1.8*LSAT + 1.4 * Essay + 1.1 * GPA + err           
...
# write.csv( df, "admissions.csv", row.names=F )
```
...
```{r}
df <- read.csv( "data/admissions.csv" )
```
...
plot(data2$GPA, data2$Admission_Prop, 
     xlab = "GPA", 
